...
CALCULATION_FORMULA_TYPES: dict[str, int] = {
    CalcType.SPREAD: 2,  # parent[0] - parent[1]
    CalcType.FLY: 3,
    CalcType.BOX: 4,
    CalcType.RATIO: 2,  # parent[0] / parent[1]
    CalcType.SPREAD_INV: 2,  # parent[1] - parent[0]
    CalcType.RATIO_INV: 2,  # parent[1] / parent[0]
    CalcType.LOG: 1,  # log-return vol: log, diff, 21d rolling std, annualized * 100
}
...
CALCULATORS_BY_TYPE: dict[str, PartitionCalculator] = {
    CalcType.SPREAD: calc_spread,
    CalcType.FLY: calc_fly,
    CalcType.BOX: calc_box,
    CalcType.RATIO: calc_ratio,
    CalcType.SPREAD_INV: calc_spread_inv,
    CalcType.RATIO_INV: calc_ratio_inv,
    CalcType.LOG: calc_log,
}
...
def parse_parent_series_codes(parent_series_code_str: str | None) -> list[str]:
...
def compute_derived_series(
    ...
    if calculator is None or required_count is None:
        logger.warning("unknown_calc_type", calc_type=calc_type_upper)
        raise UnknownCalcTypeError(f"Unknown calc_type {calc_type_upper!r}")

    if len(parent_series_codes) != required_count:
        logger.warning(
            "invalid_parent_series_count",
            calc_type=calc_type_upper,
            required=required_count,
            got=len(parent_series_codes),
        )
        raise InvalidParentSeriesCountError(
