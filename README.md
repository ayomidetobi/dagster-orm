...
EM, and CHN (CNY/CNH) variants, on top of the existing Bloomberg ingestion.
...
`market_development` metadata column -- see below). The persisted
`gold.steer_*` tables still key on a column literally named `universe` --
that's a deliberate exception (see `steer/orm.py`'s module docstring): the
Python-level concept was renamed `variant` everywhere, but the DuckLake
column wasn't, to avoid a live schema migration against real data.

Currency pairs are **not** hand-typed anywhere -- each variant's asset run
discovers every pair in that variant live from the datalake via
...
- `dagster_quickstart/steer/` -- pure business logic, layered so the math
  never touches the network: `source/` (pair/driver discovery + feature
  construction, may call the DataAPI), `analytics/` (OLS/cointegration
  estimation, signal generation, the `SteerResult` artifact -- no I/O at
  all, testable against synthetic series with no database), `config.py`
  (`StrategyConfig`/`FX_G10`/`FX_EM`/`FX_CHN`), `orm.py` (gold-table
  persistence), `model.py` (the `Steer`/`SteerPanel` facade), `run.py` (the
  `python -m dagster_quickstart.steer` CLI). No Dagster dependency anywhere
  in this tree; see `tests/test_steer_*.py` for direct unit tests against
  synthetic cointegrated/non-cointegrated series and
  `tests/test_steer_package_structure.py` for the layering guard.
- `dagster_quickstart/assets/steer/` -- the Dagster graph on top, each asset
  processing every pair in the current `variant` partition and writing one
  concatenated, `series_code`-tagged DataFrame: `steer_silver_prices`
  (discover the variant's pairs, check each one's availability -- skip
...
- `dagster_quickstart/steer/config.py` -- `FX_G10`/`FX_EM`/`FX_CHN`, one
  code-defined, frozen `FXVariant` (a `StrategyConfig` subclass) per
  variant: window, z-threshold, stop/reward ratio, logged-rate threshold,
  expected coefficient signs -- currency pairs are not configured here
  either. Each also carries the pipeline entry point
  (`FX_G10.fit(lookback_days=5, cointegration="each")` fits every pair in
  that variant -- see `steer/model.py`). The two curated global driver
  series `global_equity_series`/`commodity_series` are shared by every
  variant and live in `steer/config.py`'s `GLOBAL_DRIVERS` instead; swap
  for your own real global benchmarks if you have better ones.

### Partitioning

Partitioned by `variant` only -- `StaticPartitionsDefinition(["G10", "EM",
"CHN"])`, a fixed literal list (`assets/steer/partitions.py`). `currency_pair`
is **not** a Dagster partition dimension: each variant partition's run
discovers and fetches the complete history for every pair in that variant
in one go (`steer/source/discovery.py`'s `discover_pairs()`), and
...
a variant is resolved and assessed for driver availability within that
partition's run (`steer/source/discovery.py`'s `build_availability_report()`, consumed by
...
country-prefix series_codes, `steer/source/discovery.py`'s `COUNTRY_TO_CURRENCY`),
...
`steer/source/discovery.py`'s `EQUITY_SERIES_TO_CURRENCY`). Materialize
...
unblocked, and the entire G10 variant (33/33 pairs) is now fully unblocked,
since every G10 currency has both rate and local_equity coverage. EM and
CHN stay fully blocked -- their currencies have local_equity coverage but
no rate data yet. That's the honest, verified state of this data, not a bug
(aside from the two placeholder swap tickers, noted above). Once real rate
data exists for more currencies (even for a subset of pairs), the same
logic picks it up automatically -- see `steer/source/discovery.py`'s module docstring.
...
2. Materialize `steer_data_availability` (for the `G10`, `EM`, or `CHN`
   partition) to see the real gap report for that variant -- no
   registration step needed first, since `variant` is a static partition.
3. For a pair the report shows as available (the entire `G10` variant, 33
   ...
   `steer_signal` for that variant's partition, or materialize the whole
...
variant partition) and `steer_daily_digest_schedule` (09:30, sends the
...
variant's job with `as_of` overridden, not selecting a date partition.
`--partition` just takes the variant name directly -- no registration step,
since `variant` is a static partition:
...
pair in the variant and don't take an `as_of` themselves -- only
...
in the `G10` variant for that date in one run; loop the command over a date
